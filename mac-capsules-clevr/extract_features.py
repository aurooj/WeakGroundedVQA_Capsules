...
def main(args):
  ...
  try:
    ...
    inp2idx_mapping = {int(k):v for k,v in zip(range(min(idx_set), max(idx_set)+1), range(len(idx_set)+1))}
    print('saving image number to idx mapping..')
    with open('CLEVR_v1/data/inp2idx_mapping_val.json', "w") as outFile:
      json.dump(inp2idx_mapping, outFile)
