...
class MACCell(tf.nn.rnn_cell.RNNCell):
    ...
    def read(self, knowledgeBase, memory, control, name = "", reuse = None):
        dim = config.memDim
        ## mask
        knowledgeBase = self.queryFocusedSoftmasking(control, dim, knowledgeBase, memory, reuse)
    ...
    def queryFocusedSoftmasking(self, control, dim, knowledgeBase, memory, reuse):
        ...
        if config.combineMask:
            # mask based on both mem and control
            print("inside combine mask")
            mem_ctrl_concat = tf.concat((control, memory), axis=-1)
            print(mem_ctrl_concat.get_shape())
            control_ = ops.linear(mem_ctrl_concat, inDim=d + config.writeDim, outDim=dim,
                                  name='control_linear', reuse=reuse)  # out dim is dim based on capsules
        else:
            control_ = ops.linear(control, inDim=d, outDim=dim,
                                  name='control_linear', reuse=reuse)  # out dim is dim based on capsules
        mask_logits = ops.linear(control_, inDim=dim, outDim=config.NUM_VIS_CAPS_L2,
                                 name='mask_linear' + str(self.iteration), reuse=False)  # position aware mask
    ...
    def zero_state(self, batchSize, dtype = tf.float32):
        ...
        if config.initKBwithQ != "NON":
            ...
            cnct, dim = ops.concat(self.knowledgeBase, iVecQuestions, config.writeDim, mul = concatMul, expandY = True)
            self.knowledgeBase = ops.linear(cnct, dim, config.writeDim, name = "initKB")
