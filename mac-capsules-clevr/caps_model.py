...
import ops
from config_caps import config
from mac_capsulecell import MACCell
from capsules.capsules import create_prim_conv2d_caps, create_conv2d_caps
'''
...
The MAC network: calls the MACcells (mac_capsulecell.py) config.netLength number of times,
...
class MACnet(object):
    ...
    def outputOp(self, memory, vecQuestions, images, imageInDim):
        with tf.variable_scope("outputUnit"):            
            ...
            if config.outQuestion:
                eVecQuestions = ops.linear(vecQuestions, config.ctrlDim, config.writeDim, name = "outQuestion")
                features, dim = ops.concat(features, eVecQuestions, config.writeDim, mul = config.outQuestionMul)
                ...
                with tf.device("/gpu:{}".format(i)):
                    with tf.name_scope("tower{}".format(i)) as scope:
                        ...
                        # MAC model
                            ...
                            B, h, w, d = imageFeatures.get_shape().as_list()
                            # print(imageFeatures.get_shape().as_list())
                            h = w = 14
                            imageFeatures = tf.reshape(imageFeatures, [-1, h, w, d])

                            vis_caps = self.getVisualCapsules(imageFeatures)
                            #print(vis_caps[0].get_shape(), vis_caps[1].get_shape()  )
                            finalControl, finalMemory = self.MACnetwork(vis_caps, vecQuestions,
    ...
    '''
        transform image features to visual capsules using em routing.
        Args:
            imageFeatures of shape (B, H, W, d)

        Returns:
            vis_caps: Capsules in the tuple of (poses, activations)
                      poses of shape (B, H, W, NUM_VIS_CAPS_L2, kxk) --Note: KxK is the pose matrix dimension
                      activations of shape (B, H, W, NUM_VIS_CAPS_L2, 1)

    '''
    def getVisualCapsules(self, imageFeatures):
        prim_kb_caps = create_prim_conv2d_caps(imageFeatures, config.NUM_VIS_CAPS_L1,
                                               kernel_size=1,
                                               strides=1, padding='VALID', name='prim_caps')
        vis_caps = create_conv2d_caps(prim_kb_caps, config.NUM_VIS_CAPS_L2,
                                      kernel_size=[1, 1], strides=[1, 1],
                                      padding='VALID', name='sec_caps', route_mean=True,
                                      inv_temp=tf.cast(config.inv_temp, float),
                                      inv_temp_delta=tf.cast(config.inv_temp_delta, float)
                                      )
        return vis_caps
