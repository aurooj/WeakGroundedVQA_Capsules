...
from config_caps import config, loadDatasetConfig, parseArgs
from preprocess import Preprocesser, bold, bcolored, writeline, writelist
from caps_model import MACnet
...
def loadImageBatch(images, batch):
    ...
    try:
        ...
        with open('CLEVR_v1/data/inp2idx_mapping_val.json', "r") as inFile:
            id2idx = json.load(inFile)
        toIndex = lambda imageId: imageId
        if id2idx is not None:
            toIndex = lambda imageId: id2idx[str(imageId)]
        imageBatch = np.stack([imagesFile["features"][toIndex(imageId)] for imageId in batch["imageIds"]], axis=0)
...
Args:
    ...
    (See caps_model.py for further details).
