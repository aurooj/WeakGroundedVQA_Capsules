    ...
    def outputOp(self, memory, control, vecQuestions, images, imageInDim):
        with tf.variable_scope("outputUnit"):            
            ...
            if config.outQuestion:
                q = vecQuestions
                eQ = ops.linear(q, config.ctrlDim, config.writeDim, name = "outQuestion")
                features, dim = ops.concat(features, eQ, config.writeDim, mul = config.outQuestionMul)
                ...
                with tf.device("/gpu:{}".format(i)):
                    with tf.name_scope("tower{}".format(i)) as scope:
                        ...
                        # MAC model
                            ...
                            if config.imageObjects:
                                num_obj, _ = config.imageDims[0], config.imageDims[1]
                            ...
                            #MAC network takes visual capsules instead of grid features
